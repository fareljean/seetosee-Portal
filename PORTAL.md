...
Checkout success URL uses `?purchase=success&order=…` (see `Seeme/app/StripeClient.php`). Dashboard shows a short banner pointing at `#booth` Enter buttons.
...
- Stripe success and cancellation return to `/dashboard/?purchase=…`; there is no separate `success.html` in this source.
